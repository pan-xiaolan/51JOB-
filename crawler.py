...
database_path = "F:\\Users\\Vicky\\PycharmProjects\\51jobCrawler\\database.db"  # 数据库路径
total_page = 500  # 爬取页数
list_of_jobs = []  # 招牌信息表
...
def init_database(path):
    sql = "create table jobtable (id INTEGER primary key autoincrement, " \
          ...
          "location varchar, salary varchar , post_date varchar , demand varchar, exp varchar, " \
...
def write_to_database(file):
    ...
    for job in file:
        for i in range(len(job)):
            job[i] = ("\"" + job[i] + "\"")
        sql = 'INSERT INTO jobtable(job_name,job_link,company,company_link,location,salary,post_date,demand,exp,company_type,company_size,job_info) VALUES(%s)' % ",".join(
            job)
        print(sql)
        cur.execute(sql)
        con.commit()
